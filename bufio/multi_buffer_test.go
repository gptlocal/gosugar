...
func TestMultiBufferRead(t *testing.T) {
	b1 := New()
	lang.Must2(b1.WriteString("ab"))

	b2 := New()
	lang.Must2(b2.WriteString("cd"))
...
func TestMultiBufferSliceBySizeLarge(t *testing.T) {
	lb := make([]byte, 8*1024)
	lang.Must2(io.ReadFull(rand.Reader, lb))
...
func TestSplitFirstBytes(t *testing.T) {
	a := New()
	lang.Must2(a.WriteString("ab"))
	b := New()
	lang.Must2(b.WriteString("bc"))
